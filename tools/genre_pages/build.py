...
【いまは使っていない】2026-09-02
1枚だけ試作してチョロさんに見てもらったところ、却下になった。
理由はもっともだった。
 ・この辞典の売りは地図であって、説明文ではない
 ・別ページにすると用語の吹き出しが失われ、分からない言葉が分からないまま
   （実測で、クラウトロックの場合は吹き出しだけで6,467字を捨てていた。
     パネル本体の2,832字より多い）
 ・「たまたま地図の一部で引っかかった」という見え方にしたい

代わりに、地図の住所そのもの（genre_roots.html?genre=…）を検索に載せる
方式にした（tools/sitemap/build.py）。この道具は、その方式が
うまくいかなかったときの控えとして残してある。
